...
abstract class HardwareEffect {
  ...
  List<Color> convertToInterpolated(List<Color> colors) {
    ...
    for(var i = 0; i < colors.length; i++) {
      ...
      interpolatedColorArray.addAll(
          generateInterpolation(colors[i], colors[next], 4));
    }
...
class WaveEffect extends HardwareEffect {
  ...
  WaveEffect(int layerID, LayersProvider layersProvider) : super(layerID, layersProvider) {
    var layerColors = layersProvider.layeritems.firstWhere((element) =>
    element.id == this._layerID).mode?.currentColor.toList() as List<Color>;
  ...
  Map<String, Map<String, Object>> updateKeyboardInfo(Map<String, Map<String, Object>> keyboard) {
    ...
    while (index < zones.length && index < currentColors.length) {
      applyColorToSpecificZone(
          keyboard: keyboard, keyboardZone: zones[index], color: currentColors[index]);
      index++;
    }
    return keyboard;
  }
  ...
  int getIdx(String size) {
    ...
    var largestColumn = 0;
    layer.keys.forEach((element) {
      if(element.keyColumn < smallestColumn) smallestColumn = element.keyColumn;
  ...
  run() async {
    while(true) {
      ...
      await Future.delayed(
          Duration(milliseconds: 150), () => null);
...
class SupportContactEffect extends HardwareEffect {
  SupportContactEffect(int layerID, LayersProvider layersProvider) : super(layerID, layersProvider) {
    this.run();
  }
